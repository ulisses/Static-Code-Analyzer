    ...
    xpickle =
          ...
          xpElem "metric" $ xpPair (xpTriple
            ...
            where fromL = M.fromList . foldr step []
                  ...
                  toL   = foldr (\((a,b,c),d) t ->  ((a, maybe "" mkAbsolutePathUnsafe b, maybe "" id c),d) :t ) [] . M.toList
...
instance XmlPickler MetricValue where
    xpickle = xpAlt tag ps
        ...
        ps = [ xpWrap ( Num , \(Num i) -> i )     (xpAddFixedAttr "type" "num" $ xpAttr "value" $ xpWrap (read, show) xpText)
             ...
             , xpWrap ( FunSig , \(FunSig sigs) -> sigs) $ xpListSigs
        ...
        xpListSigs = xpElem "functionsSignatures" $ xpList (xpElem "sig" xpText)
