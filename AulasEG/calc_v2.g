	...
	:	ID '=' expression [declarations_in] 
			{
				if (!declarations_in.containsKey($ID.text)){
					...
					if (declarations_in.get($ID.text).getTipo().equals("int") && $expression.tipo == Tipo.REAL )
						System.out.println( "Possivel perda de precisão! (guardar real em variavel inteira)");
...
factor	[HashMap<String,Par> declarations_in]
	...
	| 	value[declarations_in] {tipo = $value.tipo; valor = $value.valor;System.out.println("valor na produçao factor: "+$value.valor);} 
	;
...
value  	[HashMap<String,Par> declarations_in]
	...
	@init {float valor;Tipo tipo = Tipo.INT;} 
 	:	ID {String type = declarations_in.get($ID.text).getTipo();
 		...
 		valor = declarations_in.get($ID.text).getValue();System.out.println("valor na produçao  value: "+valor);
 		} 
 		| INT {tipo = Tipo.INT; valor = Integer.parseInt($INT.text);System.out.println("valor na produçao  value: "+valor);}
 		| FLOAT {tipo = Tipo.REAL; valor = Float.parseFloat($FLOAT.text);System.out.println("valor na produçao  value: "+valor);}
 	;
